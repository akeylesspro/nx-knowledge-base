            ...
            <div className="max-w-4xl mx-auto py-8 px-6">
                ...
                <div className="mt-6 mb-8">
                    ...
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        ...
                        {meta.last_synced_at && (
                            <span className="flex items-center gap-1">
                                <i className="fa-solid fa-clock" /> <TranslatedText tKey="app.last_synced" />: {new Date(meta.last_synced_at).toLocaleDateString()}
                            </span>
                        )}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-8">
                    <div className="bg-card rounded-xl border border-border p-4 text-center">
                        ...
                        <p className="text-xs text-muted-foreground"><TranslatedText tKey="app.documented_files_label" /></p>
                    </div>
                    <div className="bg-card rounded-xl border border-border p-4 text-center">
                        <p className="text-xl font-bold">{meta.symbol_count ?? "—"}</p>
                        <p className="text-xs text-muted-foreground"><TranslatedText tKey="app.symbols_label" /></p>
                    </div>
                    <div className="bg-card rounded-xl border border-border p-4 text-center">
                        <p className="text-xl font-bold">{docFiles.length}</p>
                        <p className="text-xs text-muted-foreground"><TranslatedText tKey="app.doc_files" /></p>
                    </div>
                </div>
                ...
                {/* File listing */}
                <h2 className="text-lg font-semibold mb-4"><TranslatedText tKey="app.doc_files_title" /></h2>
                {docFiles.length === 0 ? (
                    <div className="text-center py-12 bg-card rounded-xl border border-border">
                        <i className="fa-solid fa-file-circle-question text-3xl text-muted-foreground mb-3 block" />
                        <h3 className="text-base font-medium mb-2"><TranslatedText tKey="app.no_doc_files" /></h3>
                        <p className="text-sm text-muted-foreground"><TranslatedText tKey="app.no_doc_files_hint" /></p>
                    </div>
                ) : (
                ...
                <div className="mt-8">
                    <Link href={`/repos/${repo}/api-docs`} className="inline-flex items-center gap-2 px-4 py-2.5 bg-card rounded-xl border border-border hover:shadow-md transition-shadow text-sm">
                        ...
                        <span><TranslatedText tKey="app.view_api_docs" /></span>
...
const RepoTreeRow = ({ node, repoName }: RepoTreeRowProps) => {
    ...
    return (
        <Link href={href} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/50 transition-colors">
            ...
            {isFolder && node.children ? <span className="text-xs text-muted-foreground ml-auto"><TranslatedText tKey="app.items_count" values={{ count: node.children.length }} /></span> : null}
